...
2. **Setup messaggi** — `/setup-channels` (admin) popola i canali definiti in `config/channels.config.js` con embed curati.
3. **Audio** — `/play`, `/skip`, `/stop`, `/queue` tramite `discord-player` (YouTube + Spotify, playlist incluse).
